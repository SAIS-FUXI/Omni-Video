...
def rope_apply(x, grid_sizes, freqs, sp_group=None):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        sp_size = get_world_size(sp_group)
...
def sp_dit_forward(
    ...
    context = torch.chunk(context, sp_size, dim=1)[sp_rank]
    ...
    # blocks with gradient checkpointing support
    ...
    # Context Parallel
    x = gather_forward(x, dim=1, group=sp_group)
...
def sp_cross_attn_forward(self, x, context, context_lens, dtype=torch.bfloat16):
    b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim
    ...
    # query from x (sharded sequence)
    q = self.norm_q(self.q(x)).view(b, s, n, d)
    # key, value from context (sharded sequence)
    k = self.norm_k(self.k(context)).view(b, s_ctx, n, d)
    v = self.v(context).view(b, s_ctx, n, d)

    sp_group = getattr(self, 'sp_group', None)

    # DeepSpeed Ulysses distributed attention
    x = distributed_attention(
        half(q),
        half(k),
        half(v),
